...
class TrafficSignal:
    ...
    def __init__(
        ...
        # log lane xe ưu tiên
        print("Controlled lanes:", self.lanes)
    ...
    # (new) phát hiện xe ưu tiên =============================================
    def emergency_override(self):
        """
        Nếu phát hiện xe ưu tiên thì bật pha xanh tương ứng.
        Trả về True nếu đang ưu tiên.
        """
        ...
        for lane in self.lanes:
            ...
            for veh in vehicles:
                ...
                if self.sumo.vehicle.getVehicleClass(veh) == "emergency":

                    if lane.startswith("UL2") or lane.startswith("DR2"):
                        self.sumo.trafficlight.setRedYellowGreenState(
                            self.id,
                            self.all_phases[0].state
                        )
                        self.green_phase = 0

                    elif lane.startswith("LD1") or lane.startswith("RU1"):
                        self.sumo.trafficlight.setRedYellowGreenState(
                            self.id,
                            self.all_phases[2].state
                        )
                        self.green_phase = 2
    ...
    def set_next_phase(self, new_phase: int):
        ...
        # -----------tmp-----------
        print(
            f"Time={int(self.env.sim_step):4d} "
            f"Current={self.green_phase} "
            f"Next={new_phase} "
            f"Queue={self.get_lanes_queue()}"
        )
        # ----------e_tmp----------

        # ép đèn cho xe ưu tiên ========
